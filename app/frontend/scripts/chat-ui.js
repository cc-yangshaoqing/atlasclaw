...
function buildMessageText(thinkingContent, responseContent, elapsedSeconds = null, isThinking = false) {
    ...
    if (isThinking) {
        ...
        const thinkingLabel = `ÊÄùËÄÉ‰∏≠<span class="thinking-dots"><span>.</span><span>.</span><span>.</span></span>`;

        let html = THINKING_STYLES;
        html += `<div class="thinking-block thinking">`;
        html += `<div class="thinking-header">`;
        html += `<span class="thinking-icon">‚ö?/span>`;
    ...
    if (thinkingContent && thinkingContent.trim().length > 0) {
        ...
        const labelText = timerText ? `Â∑≤ÊÄùËÄÉÔºàÁî®Êó∂ ${timerText}Ôºâ` : 'Â∑≤ÊÄùËÄ?;
                ...
                const initialHtml = `<div class="message-wrapper">${THINKING_STYLES}
                    <div class="thinking-block thinking" id="${thinkingBlockId}">
                        <div class="thinking-header">
                            ...
                            <span class="thinking-label">ÊÄùËÄÉ‰∏≠<span class="thinking-dots"><span>.</span><span>.</span><span>.</span></span></span>
                            <span class="thinking-toggle">‚Ä?/span>
                        </div>
            ...
            onEnd: () => {
                ...
                const doFinalRender = () => {
                    ...
                    if (hasRenderedDelta || hasThinkingContent) {
                        ...
                        try {
                            signals.onResponse({ html: '<div class="response-content">‚Ä?/div>', overwrite: true });
                        } catch (e) {}
                    }
